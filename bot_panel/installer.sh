...
surat=$(curl -sS https://raw.githubusercontent.com/izhanworks/izvpnauthip/main/authbot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo ""
        echo -ne "Your name store [default: SCVPS-STORE] : "
        read store_pnl
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -q -O https://raw.githubusercontent.com/izhanworks/izvpnauthip/main/scvpst.sh >/etc/.maAsiss/.Shellbtsss
			}
