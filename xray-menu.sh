...
vless://$uuid@$domain:$xtls?flow=xtls-rprx-direct&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpn123
vless://$uuid@$domain:$xtls?flow=xtls-rprx-splice&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpn123
vless://$uuid@$domain:$xtls?encryption=none&security=xtls&sni=$BUG&type=ws&host=$BUG&path=/xrayws#$user@vpn123
trojan://$uuid@$domain:$xtls?sni=$BUG#$user@vpn123
${vmesslink1}
EOF
    cat <<EOF >>"/etc/rare/config-url/${user}"
# =======================================
# XRAY CORE CONFIG MERLIN CLASH ASUS
# By vpn123 telegram: @vpn123
# =======================================
proxies:
  - {name: VLess Splice ${user}@vpn123, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-splice, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: tcp, sni: $BUG, udp: true}
  - {name: VLess Direct ${user}@vpn123, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-direct, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: tcp, sni: $BUG, udp: true}
  - {name: VLess WS ${user}@vpn123, server: $BUG.$domain, port: $xtls, type: vless, flow: xtls-rprx-direct, uuid: $uuid, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayws, ws-headers: {Host: $BUG}, sni: $BUG, udp: true}
  - {name: Trojan ${user}@vpn123, server: $domain, port: $xtls, type: trojan, password: $uuid, sni: $BUG, skip-cert-verify: true, udp: true}
  - {name: VMess ${user}@vpn123, server: $domain, port: $xtls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayvws, ws-headers: {Host: $BUG}, udp: true}
  - {name: VMess SNI ${user}@vpn123, server: $domain, port: $xtls, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: true, skip-cert-verify: true, network: ws, ws-path: /xrayvws, ws-headers: {Host: $BUG}, sni: $BUG, udp: true}
port: 3333
socks-port: 23456
redir-port: 23457
allow-lan: true
...
EOF
	...
	echo -e ""
	echo -e " Link VLESS SPLICE: vless://$uuid@$BUG.$domain:$xtls?flow=xtls-rprx-splice&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpn123"
    echo -e ""
    echo -e " Link VLESS DIRECT: vless://$uuid@$BUG.$domain:$xtls?flow=xtls-rprx-direct&encryption=none&security=xtls&sni=$BUG&type=tcp&headerType=none&host=$BUG#$user@vpn123"
    echo -e ""
	echo -e " Link VLESS WS: vless://$uuid@$BUG.$domain:$xtls?encryption=none&security=xtls&sni=$BUG&type=ws&host=$BUG&path=/xrayws#$user@vpn123"
    echo -e ""
	echo -e " Link TROJAN: trojan://$uuid@$BUG.$domain:$xtls?sni=$BUG#$user@vpn123"
    echo -e ""
